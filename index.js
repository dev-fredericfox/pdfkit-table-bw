    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        options.divider.header ||
          (options.divider.header = { disabled: false, width: undefined, opacity: undefined });
        ...
        const calcColumnSizes = () => {
          ...
          w =
            this.page.width - this.page.margins.right - (options.x || this.page.margins.left);
          // (table width) 2o - Size defined
          options.width &&
            (w = parseInt(options.width) || String(options.width).replace(/[^0-9]/g, "") >> 0);
        ...
        const addHeader = () => {
          ...
          const calc =
            startY + titleHeight + firstLineHeight + this.headerHeight + safelyMarginBottom; // * 1.3;
          ...
          } else if (calc*0.7 > maxY) { //added *0.7 - this fixes issues with tables repeating on the same page??? I just really don't understand why yet.
            // && !lockAddPage
            // lockAddHeader = false;
            lockAddPage = true;
